...
export const seedSuperAdmin = async (): Promise<void> => {
  try {
    ...
    // 5. Seed 20 Default Technicians (only if none exist to preserve manual registrations)
    const techCount = await User.countDocuments({ role: 'technician' });
    if (techCount === 0) {
      console.log('No technicians found. Seeding 20 default technicians...');
      ...
      console.log('✔ Technicians already exist. Skipping seeding.');
    }

    // 6. Seed 20 Default Customers (only if none exist to preserve manual registrations)
    const userCount = await User.countDocuments({ role: 'user' });
    if (userCount === 0) {
      console.log('No customers found. Seeding 20 default customers...');
